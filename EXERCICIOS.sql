...
SELECT
	ISNULL(SUBSTRING(CONVERT(VARCHAR(10),A.DATA_VENDA,103),4,10),'TOTAL GERAL'),
	ISNULL(D.NOME_CATEGORIA,'TOTAL MENSAL'),
	SUM(A.VALOR_TOTAL)
FROM
	VENDAS A
INNER JOIN
...
AND A.STATUS='N'
GROUP BY ROLLUP
	(SUBSTRING(CONVERT(VARCHAR(10),A.DATA_VENDA,103),4,10),D.NOME_CATEGORIA)
...
SELECT
	ISNULL(SUBSTRING(CONVERT(VARCHAR(10),A.DATA_VENDA,103),4,10),'TOTAL GERAL'),
	ISNULL(B.NOME_VENDEDOR,'TOTAL MENSAL'),
	SUM(A.VALOR_TOTAL)
FROM
	VENDAS A
INNER JOIN
...
AND A.STATUS='N'
GROUP BY ROLLUP
	(SUBSTRING(CONVERT(VARCHAR(10),A.DATA_VENDA,103),4,10),B.NOME_VENDEDOR)
...
SELECT
	TOP 10 ROW_NUMBER() OVER (ORDER BY SUM(C.VAL_TOTAL)DESC)POSICAO,
	B.NOME_PRODUTO,
	A.NOME_CATEGORIA,
	SUM(C.VAL_TOTAL)AS TOTAL
FROM
	CATEGORIA A
INNER JOIN
	PRODUTOS B
...
INNER JOIN
	VENDAS D
ON
	C.NUM_VENDA = D.NUM_VENDA
GROUP BY
	B.NOME_PRODUTO,A.NOME_CATEGORIA
...
SELECT
	...
	A.NOME_CATEGORIA,
	SUM(C.VAL_TOTAL)AS TOTAL,
	100/@TOT_G*SUM(C.VAL_TOTAL) AS PCT_PARTICIP
FROM
	CATEGORIA A
INNER JOIN
	PRODUTOS B
...
ON
	C.NUM_VENDA = D.NUM_VENDA
WHERE YEAR(D.DATA_VENDA)=@ANO AND D.STATUS='N'
GROUP BY
	B.NOME_PRODUTO,A.NOME_CATEGORIA
...
WHERE
	YEAR(A.DATA_VENDA)=2017
GROUP BY
	MONTH(A.DATA_VENDA),B.ID_PROD
ORDER BY 
	MONTH(A.DATA_VENDA) ASC, 3 DESC
...
INNER JOIN
(SELECT 
	C.ID_PROD,MAX(B.DATA_VENDA) DATA_VENDA
FROM 
	VENDAS B
INNER JOIN
	VENDA_ITENS C
ON
	B.NUM_VENDA = C.NUM_VENDA
GROUP BY
	C.ID_PROD) T1
ON
	A.ID_PROD=T1.ID_PROD